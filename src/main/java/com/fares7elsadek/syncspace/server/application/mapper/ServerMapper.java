...
public class ServerMapper {
    public ServerUserDto toServerUserDto(User user) {
    ...
    public ServerDto toServerDto(Server server) {
        return new ServerDto(server.getId(),server.getName(),server.getIconUrl(),server.getDescription(),
                server.getMembersNumber());
    }

    public ServerMemberDto toServerMemberDto(ServerMember serverMember) {
        return new ServerMemberDto(serverMember.getNickname(),serverMember.getRole().getName(),
                toServerUserDto(serverMember.getUser()));
    }
}
