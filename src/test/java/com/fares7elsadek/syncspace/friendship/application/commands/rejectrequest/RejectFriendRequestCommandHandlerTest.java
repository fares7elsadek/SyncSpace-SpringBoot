...
class RejectFriendRequestCommandHandlerTest {
    ...
    @InjectMocks
    private RejectFriendRequestCommandHandler handler;
    ...
    @Test
    @DisplayName("Should successfully reject a pending friend request")
    void shouldRejectPendingFriendRequest() {
        // Arrange
        RejectFriendRequestCommand command = new RejectFriendRequestCommand(requestId);
        ...
        assertEquals(String.format("Request with id %s has been rejected", requestId), response.message());
        assertNull(response.data());

        // Verify friendship status was updated
        assertEquals(FriendShipStatus.REJECTED, pendingFriendship.getFriendShipStatus());
        ...
        ArgumentCaptor<RejectFriendRequestEvent> eventCaptor = ArgumentCaptor.forClass(RejectFriendRequestEvent.class);
    ...
    void shouldThrowExceptionWhenUserNotAddressee() {
        ...
        assertEquals(
                String.format("User %s is not allowed to reject request %s", unauthorizedUserId, requestId),
    ...
    void shouldPublishCorrectEvent() {
        ...
        // Assert
        ArgumentCaptor<RejectFriendRequestEvent> eventCaptor =
                ArgumentCaptor.forClass(RejectFriendRequestEvent.class);
        verify(springEventPublisher).publish(eventCaptor.capture());

        RejectFriendRequestEvent publishedEvent = eventCaptor.getValue();
        assertNotNull(publishedEvent);
        // Add more assertions based on your RejectFriendRequestEvent.toEntity() implementation
    }
    ...
    @Test
    @DisplayName("Should verify transaction rollback on event publishing failure")
    void shouldHandleEventPublishingFailure() {
        ...
        doThrow(new RuntimeException("Event publishing failed"))
                .when(springEventPublisher).publish(any(RejectFriendRequestEvent.class));
    ...
    @Test
    @DisplayName("Should maintain friendship status as REJECTED after successful rejection")
    void shouldMaintainRejectedStatus() {
        // Arrange
        RejectFriendRequestCommand command = new RejectFriendRequestCommand(requestId);
        User userInfo = User.builder().id(currentUserId).build();
        ...
        when(friendshipRepository.save(friendshipCaptor.capture())).thenReturn(pendingFriendship);

        // Act
        handler.handle(command);

        // Assert
        Friendships savedFriendship = friendshipCaptor.getValue();
        assertEquals(FriendShipStatus.REJECTED, savedFriendship.getFriendShipStatus());
        assertEquals(requestId, savedFriendship.getId());
        assertEquals(requester.getId(), savedFriendship.getRequester().getId());
        assertEquals(addressee.getId(), savedFriendship.getAddressee().getId());
    }
}